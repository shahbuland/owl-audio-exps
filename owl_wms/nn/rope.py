...
class AVRoPE(nn.Module):
    ...
    def __init__(self, config):
        ...
        base_freqs = RotaryEmbedding(
            dim=head_dim // 4,  # Using half dimension since we only need 1D rotation
            freqs_for='pixel',
            max_freq=256
        ).get_axial_freqs(config.n_frames, p + 1, p + 1)

        vid_freqs = base_freqs[:, :p, :p].reshape(config.n_frames, p**2, -1)  # top left square
    ...
    def forward(self, x_vidio, x_audio, offset: int = 0):
        ...
        cos = self.cos[..., offset:offset + x.size(2), :]   # keep leading dims for broadcast
        sin = self.sin[..., offset:offset + x.size(2), :]
        x_rot, x_rest = x[..., :self.rot_feats], x[..., self.rot_feats:]
        r0, r1 = x_rot.float().unfold(-1, 2, 2).unbind(-1)
        y_rot = torch.cat((r0 * cos - r1 * sin, r1 * cos + r0 * sin), dim=-1).type_as(x)
        y = torch.cat((y_rot, x_rest), dim=-1)
